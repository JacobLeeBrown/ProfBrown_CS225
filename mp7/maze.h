 ...
 * solution. (Note that by "square maze" we mean a maze in which each cell is a 
 * square; the maze itself need not be a square.)
 */
class SquareMaze
{
	public:
		 ...
		 * are no more walls that could be deleted without creating a cycle. Do 
		 * not delete walls on the perimeter of the grid.
		 *
		 ...
		 * be used by the grading program to verify that your maze is a tree 
		 * that occupies the whole grid, and to verify your maze solution. So 
		 ...
		 */
		int canTravel(int x, int y, int dir) const;
		 ...
		 * the other member functions. setWall should not prevent cycles from 
		 ...
		 *					wall to set (same as the encoding explained in 
		 * 					canTravel). You only need to support setting the 
		 ...
		 * as the length (measured as a number of steps) of the only path 
		 ...
		 * integers, where each integer represents the direction of a step, 
		 ...
		 * corresponds to a trail of 11 red pixels in the given direction. If 
		 * the first step is downward, color pixels (5,5) through (5,15) red. 
		 * (Red is 255,0,0 in RGB). Then if the second step is right, color 
		 * pixels (5,15) through (15,15) red. Then if the third step is up, 
		 * color pixels (15,15) through (15,5) red. Continue in this manner 
		 * until you get to the end of the solution vector, so that your output 
	...
	private:
		/** Store the height and width of this SquareMaze instance */
		int _width;
		int _height;
		int bestX;
		/** Private member storage for the maze */
		DisjointSets cells;
		vector<bool> walls_d; // Bottoms walls of cells
		vector<bool> walls_r; // Right walls of cells

		/**
		 * Private helper function for setting walls of the private member 
		 * storage from within the public member function makeMaze.
		 */
		void setWalls();

		/**
		 * Private helper function for checking that removing the designated 
		 * wall of a cell is okay (will not remove border wall (#trump2k16) and 
		 ...
		 *				to check
		 *
		 * return 		Whether or not it is okay to remove the wall
		 */
		bool wallCheck(int x, int y, int dir);

		/**
		 * Private helper function for making sure that there is only one set
		 * that represents the entire maze, and not multiple disjoint sets.
		 *
		 * This is basically a non-random setWalls function, but since it is
		 * called after setWalls, we should still be gucci.
		 */
		void unionStragglers();

		/**
		 * Private member function for making sure the vectors are set properly 
		 * within the public member function makeMaze.
		 */
		void resetVectors();

		/**
		 * Private recursive helper function for assisting solveMaze
		 *
		 * @param x 		The x coordinate of the current cell
		 * @param y 		The y coordinate of the current cell
		 * @param fromDir 	The direction we came from to get to current cell
		 * @param curPath 	The vector of directions we've take to this cell
		 * @param curBest	The vector containing the current longest path
		 */
		void solveMaze(int x, int y, int dir, vector<int> & curPath,
					   vector<int> & curBest);
};

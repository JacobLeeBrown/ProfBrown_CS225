...
{
    // Easy check for non-anagrams
	if(word.length() != test.length()) return false;

	// Instantiate hashtable of arbitrary size 32
	Dict<char, int> hashTable(32);
	// For each character in word, incremement its frequency in the hashtable
	for(size_t i = 0; i < word.length(); i++)
	{
		hashTable[word[i]]++;
	}
	// For each character in test, decrement its frequency in the hashtable
	for(size_t i = 0; i < test.length(); i++)
	{
		hashTable[word[i]]--;
		// If the freqency falls below zero, there is a mismatch of characters, thus not an anagram
		if(hashTable[word[i]] < 0) return false;
	}

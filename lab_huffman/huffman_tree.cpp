...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     *
     * Finally, when there is a single node left, it as the root. Assign it
    ...
    while(singleQueue.size()+mergeQueue.size() > 1)
    {
		...
		TreeNode* newParent = new TreeNode(smallest1->freq.getFrequency() + 
										   smallest2->freq.getFrequency());
		newParent->left = smallest1;
		newParent->right = smallest2;
		mergeQueue.push(newParent);
	}
...
bool HuffmanTree::isLeaf(TreeNode* n)
{

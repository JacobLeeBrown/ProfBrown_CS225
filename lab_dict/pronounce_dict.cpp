...
{
    // Store both of the parameters as local, modifiable variables.
    string w1 = word1;
    string w2 = word2;
    // Turn w1 and w2 into completely uppercase
    std::transform(w1.begin(), w1.end(), w1.begin(), ::toupper);
    std::transform(w2.begin(), w2.end(), w2.begin(), ::toupper);
    // Lookup each word in the dict
    auto lookup1 = dict.find(w1);
    auto lookup2 = dict.find(w2);
    // Make sure both words are in the dict
    if(lookup1 == dict.end() || lookup2 == dict.end())
    {
    	return false;
    }
    // They're both in the dict, so return if they are equal or not

...
class BTree
{
  private:
    ...
    struct BTreeNode {
        ...
        {
        	// CHANGED because values *were* written such that order = max # of *elements*, not *children*
...
{
	// Check is elements is empty
	if(elements.empty())
	{
		//std::cout << "Check 1" << std::endl;
		return 0;
	}
	// Check if val should go before the first element
	else if(val < elements.front())
	{
		//std::cout << "Check 2" << std::endl;
		return 0;
	}
	// Check if val should go after the last element
	...
	while(start <= end)
	{
		...
		//std::cout << "(Start, End, Mid) = (" << start << ", " << end << ", " << mid << ")" << std::endl;
		if(elements[mid] == val)
		{
			return mid;  
		}
		...
		{
			start = mid + 1;
			//std::cout << "Updated start to: " << start << std::endl;
		}
		else
		{
			end = mid - 1;
			//std::cout << "Updated end to: " << end << std::endl;
		}
	}
	return start;
}
...
template <class T, class C>
bool chk_for_key(const std::vector<T>& elements, const size_t idx, const C& val)
{
	if(elements.size() == 0 || idx >= elements.size())
	{
		return false;
	}
	else if(elements.at(idx) == val)
	{
		return true;
	}
	else
